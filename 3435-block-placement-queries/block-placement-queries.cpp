...
private:
    ...
    void buildSegTree(int ind, int l, int r, vector<int>& arr,
        ...
        segTree[ind].rangeSum =
            segTree[2 * ind + 1].rangeSum + segTree[2 * ind + 2].rangeSum;
        segTree[ind].prefMaxSum = max(segTree[2 * ind + 1].prefMaxSum,
                                      segTree[2 * ind + 1].rangeSum +
                                          segTree[2 * ind + 2].prefMaxSum);
        segTree[ind].suffMaxSum =
            max(segTree[2 * ind + 1].suffMaxSum + segTree[2 * ind + 2].rangeSum,
                segTree[2 * ind + 2].suffMaxSum);
        segTree[ind].maxSubArrSum = max({segTree[2 * ind + 1].maxSubArrSum,
                                         segTree[2 * ind + 2].maxSubArrSum,
                                         segTree[2 * ind + 1].suffMaxSum +
                                             segTree[2 * ind + 2].prefMaxSum});
    }

    void pointUpdate(int ind, int l, int r, int pos, int newVal,
                     vector<SegTreeNode>& segTree) {
        if (l == r) {
    ...
    SegTreeNode getRangeMaxSubSum(int ind, int l, int r, int start, int end,
                                  vector<SegTreeNode>& segTree) {
        if (l > end || r < start)
        ...
        SegTreeNode leftNode =
            getRangeMaxSubSum(2 * ind + 1, l, mid, start, end, segTree);
        SegTreeNode rightNode =
            getRangeMaxSubSum(2 * ind + 2, mid + 1, r, start, end, segTree);

        return SegTreeNode(
            leftNode.rangeSum + rightNode.rangeSum,
            ...
            max({leftNode.maxSubArrSum, rightNode.maxSubArrSum,
                 leftNode.suffMaxSum + rightNode.prefMaxSum}));
    }
...
public:
    vector<bool> getResults(vector<vector<int>>& queries) {
        ...
        for (int i = 0; i < n; i++) {
            if (queries[i][0] == 1)
                pointUpdate(0, 0, 2 * max_dist - 1, 2 * queries[i][1] - 1, -1e8,
                            segTree);
            else
                ans.push_back(queries[i][2] <=
                              getRangeMaxSubSum(0, 0, 2 * max_dist - 1, 0,
                                                2 * queries[i][1] - 1, segTree)
                                  .maxSubArrSum);
        }
